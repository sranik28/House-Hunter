...
const ManageAllBooking = () => {
    ...
    const { data: booking = [], refetch } = useQuery({
        ...
        enabled: !loading,
        queryFn: async () => {
          const response = await axios.get(`https://house-hunter-backend-caxj15ubr-sranik28.vercel.app/bookedhouse/${user?.email}`);
  ...
  const handleApproved = (book) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch(`https://house-hunter-backend-caxj15ubr-sranik28.vercel.app/approvedbooking/${book._id}`)
        .then(res => {
